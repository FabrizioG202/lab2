...
#place(
  ...
  [
    ...
    #text(weight: "bold")[Results:] With this paper, we provide three models for the detection of Signal Peptides from sequence information. The first, based on the Von-Heijne algorithm, produces a relatively accurate classifier, with an MCC of $0.66$. The second model is based on Support Vector Machines and achieves an higher MCC of $0.86$ and $f_1$ score of $0.87$, supporting an overall stronger model. Given the complexity of the full SVM model, we also trained a reduced model, which trades some performance (MCC of $0.73$, and $f_1$ of $0.76$), for a significantly lower number of input features, lowering the amount of preprocessing needed and the overall complexity of the final model. We also present a predictor based on Protein Language Models, which behaves as a near perfect classifier, with an MCC of $0.96$ and $f_1$ score of $0.96$, but is not directly comparable to the other two models, given the much higher complexity of the underlying method. #linebreak()
...
= Introduction
In both eukaryotic and prokaryotic cells, for a protein to enter the secretory pathway, it must contain a specific targeting signal. Often, this signal is a short sequence located at the N-terminus of proteins @von_Heijne_1990. The signal peptide (SP) has a distinct three-domain structure (depicted in @sp_structure) with a positively charged N-terminal region (N-region), a central hydrophobic $alpha$-helical core (H-region) and a more polar C-terminal region (C-region) containing the cleavage site @von_Heijne_1990. After completing its mission as protein 'zip code', the short peptide is cleaved by a membrane-bound peptidase, which separates it from the mature protein @von_Heijne_1998. Amino acid substitutions in the C-region revealed a universally conserved recognition site for the leader peptidase, with the three-residue motif Ala#super("-3")-X-Ala#super("-1"), where X can be any amino acid and numbering is relative to the cleavage site @Kuhn_Wickner_1985.

Given their importance in many areas of molecular cell biology such as targeted drug design @Lumangtad_Bell_2020, the accurate detection of SPs remains a crucial task in bioinformatics and has been addressed using a variety of approaches, including machine learning (SVMs @Nugent_Jones_2009, @Petersen_Brunak_von_Heijne_Nielsen_2011 and Bayesian networks @Reynolds_Käll_Riffle_Bilmes_Noble_2008, Hidden Markov Models @Käll_Krogh_Sonnhammer_2004), and, more recently, deep learning @Savojardo_Martelli_Fariselli_Casadio_2018.
...
In this work, we propose a comparative analysis of different approaches for the detection of SPs from protein sequences. We compare the performance of the Von-Heijne model, which is based on a position-specific weight matrix (PSWM) generated from the context around the cleavage site, with that of a Support Vector Machine (SVM) model trained on a variety of features extracted from the sequence and a simple Multi-Layer Perceptron (MLP) trained on sequence embeddings computed using a PLM.


= Materials and Methods
== Data Collection
Reviewed, non-fragment, protein sequences from eukaryotic taxa were queried from UniprotKB @UniProt_Consortium_2018, and collected into 2 distinct sets: positive and negative. For the positive set, we selected sequences with experimental evidence for the presence of a signal peptide (`ft_signal_exp:*`)
For the negative set, we selected proteins not annotated with a signal peptide but having experimental evidence for subcellular localization in non-secretory comparments, namely nucleus (`cc_scl_term_exp:SL-0191`), peroxisome (`cc_scl_term_exp:SL-0204`), cell membrane / plasma membrane (`cc_scl_term_exp:SL-0039`), cytosol (`cc_scl_term_exp:SL-0091`), plastid (`cc_scl_term_exp:SL-0209`) or mitochondrion (`cc_scl_term_exp:SL-0173`). For sequences in the negative set, we gathered information about the presence of transmembrane (TM) helices, whose hydrophobicity profile mimicks those of signal peptides.

Further selection on the positive entries was carried out in order to select proteins which had informaton about the cleavage site and a Signal Peptide with a length of at least 14 residues.
...
Sequences were clustered using mmseqs2 @Steinegger_Söding_2017, using parameters `--min-seq-id 0.3, cluster-mode 1 -cov-mode 0 -c 0.4`, in order to cluster sequences with a similarity of at least 30\% and an alignment spanning at least 40\% of the total sequence length. Then, only representative sequences from each cluster were selected for downstream analysis. The number of sequences, before and after clustering is reported in @seq-counts.
...
The full dataset was split into a training and a test set with a ratio of $0.8$. A Position-Specific Weight Matrix (PSWM) was then generated using 16-residue wide windows (13aa upstream to 2aa downstream) surrounding the cleavage site. Amino acid frequencies were normalized using background amino acid frequencies from SwissProt. After initializing the matrix with pseudocounts of 1, the weights are computed as follows:

$
  w(a, i) = log_2(f(a, i) / f(a))
$

where $f(a, i)$ is the frequency of amino acid $a$ at position $i$ in the training set, and $f(a)$ is the background frequency of amino acid $a$ in Swiss-Prot. The score for a given sequence is then computed as:
$
  S = max_(j=1)^(90-16+1) sum_(i=1)^(16) w(a_(j+i-1), i)
$

Scores are computed over the first 90 amino acids of the given sequence, using a sliding 16-reside window. A sequence is labeled as positive for an SP if the Score $S$ is greater than a threshold $t$.
...
== Feature Extraction
To build a model that can handle sequence data, sequences must be encoded as numerical representation. There are many possible ways of extracting meaningful numerical features from a sequence, for both the full and reduced SVM model, we employed 2 types of features:
- Amino acid composition up to a cutoff $k$ (`n_term_composition`) and from $k$ to an hardcoded limit $N$ (set to $90$) (`c_term_composition`)
- Protein Scales computed over the first $N$ aminoacids with a window size of $5$ (@feature-example). The hydrophobicity scale (@Kyte_Doolittle_1982), alpha-helix tendency scale (@Chou_Fasman_1978), transmembrane tendency scale (@Zhao_London_2009), bulkiness scale (@Zimmerman_Eliezer_Simha_1968), and polarity scale(@Zimmerman_Eliezer_Simha_1968) were used. For each scale, the minimum, maximum, average, standard deviation and position of maximum were used as features.
...
We used a grid search to find optimal hyperparameters, testing both RBF and linear kernels. For the RBF kernel, we evaluated C values of $0.1$, $1$, $2$, $4$, and $8$, combined with gamma values of "scale", $0.001$, $0.01$, $0.1$, $1$, and $2$. For linear kernels, we tested C values of $0.1$, $1$, $2$, $4$, and $8$. Each model was fitted and evaluated on an 80\% training split. The model yielding the highest $f_1$ score was found to have a `rbf` kernel, with $C=2$ and $gamma=0.01$, with a mean $f_1$ score across the 5 folds ran on the training set of $0.87$, and was thus selected for downstream analysis.
...
= Results
== SP Motifs
From all SP-endowed sequences, a motif logo (@sp_motif) was generated using the context around the cleavage site (13aa upstream to 2aa downstream). The motif shows the predominance of small and neutral residues, such as Alanine, in positions -1 and -3, which has already been noted in previous literature @von_Heijne_1983 and is consistent with the suggested Ala#super("-3")-X-Ala#super("-1") signature. Moreover, the high frequency of hydrophobic residues (Leucine, Valine, Phenylalanine) is consistent with the structure of SPs presented above, particularly the H-Region, which holds an hydrophobic core.

#figure(
  image(".imgs/positive_logo.svg"),

  caption: [Motif logo for the context (-13aa to 2aa) surrounding the cleavage site, showing the conserved Ala#super("-3")-X-Ala#super("-1") motif. The cleavage site is beteween residues $-1$ and $0$],
)<sp_motif>

== Von Heijne Model

For the Von-Hejne model, the computed optimal threshold was $9.25$, confusion matrix computed on the test set is reported in @vh-confusion-matrix. The model achieved an accuracy of $0.9378$, precision of $0.7474$, recall of $0.6532$, $f_1$ score of $0.6971$, and MCC of $0.6645$. The model reaches an high accuracy, but performance on the positive class is limited, detecting only around 65% of actual positives. This is likely due to both the simplicity of the model, which makes it unable to capture long-range or high-level patterns and the limited nature of the input data, which is based on a small window around the cleavage site, which may not capture all the relevant information for SP detection.
...
We analysed the set of false Positives (wrongly classified as having a Signal Peptide) of the von-heijne method and as expected, we found it to be significantly enriched (Fisher's exact test, one-sided, $"OR" = 6.45$, $"p-value"=9.45 times 10^{-11}$) for proteins having a TM helix. This is likely due to the fact that the hydrophobicity profile of a TM helix mimicks that one of a Signal Peptide.

== Support Vector Machine
The model, as chosen in the hyperparameter tuning phase, is a SVM with RBF kernel, $C=2$ and $gamma=0.01$. The confusion matrix computed on the test set is reported in @svm-confusion-matrix. The model achieved an accuracy of $0.9743$, precision of $0.9034$, recall of $0.8539$, $f_1$ score of $0.8779$, and MCC of $0.8640$. The SVM model thus outperforms the Von-Heijne model across all metrics, showing particularly strong improvement in recall (85.39% vs 65.32%), indicating better detection of signal peptides. This means that the SVM model is both more reliable when predicting positives and much better at detecting actual positive cases. It is worth noting that this increase in performance mirrors a great increase in complexity of the underlying method, which is structurally more intricate than a simple PSWM, and also requires far greater preprocessing to compute the features. To mitigate this, we tried producing a reduced model, using only the top 5 features selected using permutation importance and the top 5 features selected using the feature importance of a Random Forest Classifier, which we trained on the same dataset.
...
The top 5 features identified by each method are shown in @permutation-importance and @rf-importance. Both methods consistently identified `n_terminal_comp_L` (leucine composition in the N-terminal region) and `tm_max` (maximum transmembrane tendency) as important features, albeit with different ranks. The two methods of detecting feature importance differ with respect to the measured metrics, but the presence of the attributes in both rankings strengthens confidence that these are genuinely informative. Leucine Composition's importance is consistent with signal peptides containing a hydrophobic core, where Leucines, and more generally hydrophobic residues, such as Alanine are commonly enriched. Furthermore, the presnece of `n_terminal_comp_D`, a hydrophilic aminoacid, may be representative of the fact that the model is informed by its general depletion.
...
= Conclusion
The results show the existence of a clear trade-off between simplicity and performance. The Von-Hejne predictor, despite its simplicity, reaches an acceptable accuracy, but is plagued by a limited recall, meaning it fails at recognizing a significant portion of actual positives. Both models based on Support Vector Machines display an increased overall performance, with the full model reaching an $f_1$ score of $0.8779$, and the reduced model reaching an $f_1$ score of $0.7640$. This performance increase is likely due to the richer nature of the input features, which nicely describe physicochemical features that sequence information alone fails to capture. The reduced model, while showing a decrease in performance compared to the full model, still outperforms the Von-Heijne model, while using only 7 features instead of the full feature set, which makes it a good candidate for applications where computational resources are limited. Another aspect of the comparison that stands out is the loss in interpretability that exists with larger and more complex models. The very structure underpinning the von heijne model, that of a positional matrix, makes it biologically interpretable, as it allows to easily identify conserved positions and amino acid, and can be related to known peptidase recognition motifs. On the other hand, the SVM models are more difficult to interpret, as they rely on a complex combination of features and non-linear decision boundaries.
